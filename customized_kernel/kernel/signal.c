...
int kill_inheritance_logic(task_t* sender, task_t* receiver){
    printk("KILL LOGIC: ENTERED\n");

    if (sender->is_privileged==0 && receiver->is_privileged==0){
        printk("kill_inheritance_logic: A.P==0&&B.P==0. use regular return 0\n");
        return 0;
    }
    if (sender->is_privileged==0 && receiver->is_privileged==1){
        printk("kill_inheritance_logic: A.P==0&&B.P==1. A.P=B.P=1 return 1\n");
        //
    ...
    if (sender->is_privileged==1 && receiver->is_privileged==1){
        printk("kill_inheritance_logic: A.P==1&&B.P==1. check if A upgrades return 1\n");
